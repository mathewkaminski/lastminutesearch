...
def render() -> None:
    ...
    if st.button("Scan for duplicates"):
        try:
            ...
            st.session_state["league_dup_groups"] = groups
        except Exception as e:
            st.error(f"Scan failed: {e}")
            return

    groups = st.session_state.get("league_dup_groups")
    if groups is None:
    ...
    for i, group in enumerate(groups):
        ...
        with st.expander(
            ...
            with ca:
                if st.button("Keep Both", key=f"lg_keep_{i}"):
                    st.session_state["league_dup_groups"] = [
            ...
            with cb:
                if st.button("Merge (keep A, archive B)", key=f"lg_merge_{i}"):
            ...
            with cc:
                if st.button("Archive B", key=f"lg_del_{i}"):
